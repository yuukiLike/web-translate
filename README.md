...
面向 Chrome 桌面版的中英双语网页翻译扩展。保留原文，在正文附近插入可选择的译文；翻译优先级跟随你的阅读位置，并持续补译滚动、懒加载或 SPA 路由切换后出现的正文。
...
- 阅读优先：每批请求发送前重新判断当前视口，跳到文章后部时优先处理正在阅读的段落
- 滚动补译：初次扫描时暂未显示的正文，会在滚动、窗口尺寸或显隐变化后自动补译；支持页面内部的滚动区域
- 长段落完整显示：等待各部分翻译完成后统一展示，失败时保留原文并提示错误
- 持续识别 SPA、无限滚动和懒加载中新出现的稳定正文，跳过反复变化的轮播、计数器和动画文本
...
- 回归测试覆盖滚动补译、跳读后的优先级、长段落完整性、取消和动态内容恢复。
- 生成文件不要手工修改。代码入口和翻译流程见 [极简开发说明](docs/README.md)。

## 文档

- [极简开发说明](docs/README.md)：开发命令、代码入口、翻译流程与维护边界。
- [设计规范](DESIGN.md)：界面语言、颜色、排版和组件约束。
