...
```text
apps/bilingual-web-translator
```

仓库已经包含 Chrome 可直接加载的生成产物，因此只想安装和体验扩展时，不需要安装依赖或执行构建。参与开发、修改模型目录或 Provider 运行时前，则需要准备 Node 依赖并重新生成产物。当前 `manifest.json` 声明的扩展版本是 `0.4.0`，最低 Chrome 版本是 140。
...
- 已在本仓库根目录中找到 `apps/bilingual-web-translator`。
...
如果你要修改 `src/provider-runtime.js`、固定模型 snapshot、Provider allowlist 或对应 schema，请先准备 Node.js 22+，再在仓库根目录运行：
...
第一条命令安装构建依赖，但不运行依赖包的生命周期脚本；第二条命令先校验 snapshot、schema 和 allowlist，再生成 `lib/provider-catalog.generated.js` 与浏览器可执行的 `lib/provider-runtime.js`。这两个生成文件已经随仓库提供，因此普通安装者无需重复构建。
...
4. 选择下面这个目录，而不是它的父目录，也不是单独的 `manifest.json`：
   ...
   ```text
   <仓库根目录>/apps/bilingual-web-translator
   ```

Chrome 会把该目录根部的 `manifest.json` 当作扩展入口。
...
1. 在“翻译服务”中选择 Azure Translator、DeepL、DeepSeek、OpenAI、Google Gemini 或 Anthropic。
2. 只在密码输入框中填写你自己的 API Key。
3. 如果使用四个模型 Provider 之一，从本地固定目录提供的下拉框中选择模型。当前每个 Provider 都只有经过校验和 allowlist 允许的模型，不提供自定义 Base URL 或任意模型 ID 输入。
4. 点击“测试当前服务”。
5. 等待页面显示“连接成功”。
...
这里没有弹窗。项目没有声明 `action.default_popup`，因此左键触发 `chrome.action.onClicked`。后台先注入 `content.css`，再按顺序注入 `lib/provider-catalog.generated.js`、`lib/core.js` 和 `content.js`。第二次注入时，页面中已有的内容脚本控制器会执行“停止并恢复”。模型 SDK 的 `lib/provider-runtime.js` 只由 Service Worker 加载，不会进入网页内容脚本。
...
6. 测试结束后取消勾选“开发调试模式”，或在设置页关闭“记录调试事件”并保存。
...
普通网页                                  扩展自己的安全源
┌─────────────────────┐                 ┌──────────────────────────┐
│ content.js          │  一次性消息      │ background.js            │
...
`manifest.json` 将 `background.js` 注册为模块型 Service Worker：
...
当前后台还承担配置与模型 allowlist 检查、输入验证、批处理、缓存、超时、重试、用量统计和 Provider 响应校验。Azure Translator 与 DeepL 使用后台中的专用 REST 调用；DeepSeek、OpenAI、Google 和 Anthropic 通过打包到 `lib/provider-runtime.js` 的 Vercel AI SDK 适配器调用固定官方 API。把跨域请求放在这里，而不是内容脚本里，可以让 API Key 留在扩展受信任上下文，并由 host permission 控制访问目标。

### 2.5 Content script：能碰 DOM，但处在隔离世界

内容脚本运行在网页上下文中，可以读取和修改共享 DOM，但它的 JavaScript 全局环境与网页自身脚本隔离。网页脚本不能直接读取 `content.js` 中的局部变量；两者仍然会看到对同一 DOM 的修改。

本项目使用 `chrome.scripting` 动态注入：

1. `content.css` 定义译文和状态节点样式。
2. `lib/provider-catalog.generated.js` 在隔离世界中提供冻结的本地 Provider 与模型目录。
3. `lib/core.js` 基于该目录建立设置和纯函数工具。
4. `content.js` 遍历文本节点、选择翻译单元、发送批次并插入译文。
...
`options_ui.page` 指向 `options.html`，`open_in_tab: true` 表示设置页在独立标签页打开。`chrome.runtime.openOptionsPage()` 会打开或聚焦它。

设置页负责：

- 读取和保存 Provider、API Key、模型、翻译方向及并发数。
- 从包内 `lib/provider-catalog.generated.js` 呈现固定 Provider、模型、成本和上下文信息。
- 测试连接、读取用量和清理缓存。
- 通过长连接 Port 接收脱敏实时调试事件。

设置页不会联网刷新模型目录，也没有 Base URL 输入。目录来源 commit 和 snapshot 抓取时间只是本地产物中的元数据；更新目录必须修改仓库数据、通过 schema/allowlist 校验、重新构建并重新加载扩展。
...
| `host_permissions` | 六类 Provider 的七个固定 API origin | 安装时随清单声明 | 允许扩展 Service Worker 发起跨域 `fetch()` |

网页权限和 API 权限是两条独立边界。`activeTab` 不会自动允许请求 Provider；API host permission 也不会让扩展永久读取所有网页。

当前清单没有 `optional_host_permissions`。所有可访问 API 域名都在 `host_permissions` 中精确列出；设置页不会在运行时申请新 origin，也不能指向任意兼容服务。
...
因此 `options.html` 只加载包内的 `lib/provider-catalog.generated.js`、`lib/core.js` 和 `options.js`；Service Worker 也只导入包内代码。Vercel AI SDK 及四个 Provider 适配器在开发阶段由 esbuild 打包为本地 `lib/provider-runtime.js`，运行时不会从 npm 或 CDN 加载代码。不要改成 CDN 脚本、`eval()` 或把网络响应当 JavaScript 执行。
...
如果这是首次准备该目录，先执行 `npm install --ignore-scripts`。构建成功会明确报告已生成 `lib/provider-catalog.generated.js` 与 `lib/provider-runtime.js`；随后仍需在 Chrome 中重新加载扩展。

之后修改文件时，不需要删除后重新选择目录。根据修改位置执行：

| 修改文件 | 需要重新加载扩展 | 还需要刷新页面 |
| --- | --- | --- |
| `manifest.json` | 是 | 视测试目标而定 |
| `background.js` | 是 | 通常重新触发动作即可；为避免旧上下文干扰，建议刷新测试网页 |
| `content.js`、`content.css` | 是 | 是。旧内容脚本已进入页面，必须刷新宿主网页再点击图标 |
| `lib/provider-catalog.generated.js`、`lib/core.js` | 是 | 是。它们同时被 Service Worker、内容脚本和设置页使用 |
| `lib/provider-runtime.js` | 是 | 建议刷新测试网页；它只在 Service Worker 中执行 |
| `options.html`、`options.js`、`options.css` | Chrome 官方流程不要求重新加载整个扩展 | 刷新设置页标签 |
...
修改这些文件后必须先重新运行构建脚本，再重新加载扩展。不要直接编辑两个 `lib/` 生成产物；下一次构建会覆盖手工修改。`npm run check` 会重新计算预期产物并拒绝过期 bundle；构建器还会预设 Zod `jitless`，避免在 Manifest V3 CSP 下尝试动态代码生成。
...
不要只刷新网页来测试新的 `background.js`，也不要只重新加载扩展却保留已经注入旧 `content.js` 的网页。

### 3.2 如何核对当前运行版本

先区分两个值：

- 源码期望版本：`manifest.json` 中的 `version`。
...
3. 在 Sources 中找到注入的 `content.js` 并设置断点。
4. 在 Elements 中检查：
...
内容脚本错误出现在网页 DevTools，而不是一定出现在 `chrome://extensions` 的错误列表中。Provider 请求则由 `background.js` 发起，应到 Service Worker DevTools 查 Network。

### 3.5 如何调试设置页

1. 右键扩展图标，选择“打开详细调试面板”。
2. 在设置页标签中打开普通 DevTools。
3. 用 Console 调试 `options.js`，用 Network 区分设置页自身资源和后台请求。
...
| --- | --- | --- | --- |
| [`manifest.json`](../manifest.json) | Chrome 扩展平台 | Manifest V3 | 声明版本、最低 Chrome 版本、Service Worker、action、快捷键、设置页与权限 |
...
| [`src/provider-runtime.js`](../src/provider-runtime.js) | 构建输入 | Vercel AI SDK 适配层 | 显式创建 DeepSeek、OpenAI、Google、Anthropic 模型，采集 SDK HTTP 与响应元数据 |
| [`lib/provider-catalog.generated.js`](../lib/provider-catalog.generated.js) | Worker、内容脚本、设置页都会加载 | 包内生成数据 | 提供深度冻结的固定 Provider 和模型目录；不要手工编辑 |
| [`lib/provider-runtime.js`](../lib/provider-runtime.js) | Extension Service Worker | 包内生成代码 | 包含 Vercel AI SDK 和四个 Provider 适配器；不要手工编辑，也不注入网页 |
| [`background.js`](../background.js) | Extension Service Worker | `background.service_worker`、事件、跨域请求 | 左键入口、右键菜单、消息路由、allowlist 检查、Key、缓存、Provider 请求、重试、用量和脱敏事件 |
| [`lib/core.js`](../lib/core.js) | Worker、内容脚本、设置页都会加载 | 包内共享代码 | 基于本地目录进行设置规范化、Provider 定义、语言判断、分批、缓存签名和模型 JSON 校验 |
| [`content.js`](../content.js) | 网页中的隔离世界 | 动态 content script | 扫描 DOM、视口优先调度、监听动态内容、发消息、用 `textContent` 插入和移除译文 |
| [`content.css`](../content.css) | 注入到当前网页 | `scripting.insertCSS()` | 译文和状态节点样式；使用 `data-bt-owned` 限定扩展节点 |
| [`options.html`](../options.html) | `chrome-extension://` 页面 | Options page | 设置、用量、调试面板的结构 |
| [`options.js`](../options.js) | 受信任扩展页面 | 消息、Port、Storage 变更 | 从本地目录渲染模型、保存设置、连接测试和脱敏事件渲染 |
| [`options.css`](../options.css) | 受信任扩展页面 | Extension page UI | 设置页布局和样式 |
...
| `background.service_worker` | `background.js` | 后台事件入口 |
...
| `options_ui.page` | `options.html` | 设置页入口 |
...
| `scripting` | `toggleTranslation()` | 动态注入 CSS、`lib/provider-catalog.generated.js`、`lib/core.js` 和 `content.js` |
...
清单没有 `optional_host_permissions`，也没有用于任意 HTTPS 或本机服务的通配模式。模型 Provider 的 Base URL 由 allowlist 固定，设置页无法更改。Models.dev 不在 host permissions 中，因为运行中的扩展不向它发起目录请求。

### 4.5 关键消息接口速查

| 来源 | 消息或连接 | 用途 |
| --- | --- | --- |
| `content.js` | `START_RUN` | 建立任务快照，只取回非敏感公开设置 |
| `content.js` | `TRANSLATE_BATCH` | 发送已筛选和分批的文本，接收 ID 对齐的译文 |
| `content.js` | `CANCEL_RUN` | 第二次点击时取消请求并清理任务 |
| `content.js` | `STATUS` | 更新当前标签页徽标和 title |
| `content.js` | `OPEN_OPTIONS` | 从网页状态提示打开设置页 |
| `options.js` | `GET_OPTIONS_STATE`、`SAVE_SETTINGS`、`TEST_PROVIDER` | 管理完整设置、用量和连接测试 |
| `options.js` | `GET_DEBUG_LOGS`、`CLEAR_DEBUG_LOGS` | 读取或清空脱敏事件 |
| `options.js` | Port `debug-events-v1` | 接收实时事件、快照与重置通知 |
...
| 症状 | 最可能原因 | 处理方式 |
| --- | --- | --- |
| “加载已解压”失败 | 选错目录、Manifest JSON 错误、未知权限或 Chrome 版本低于 140 | 选择包含 `manifest.json` 的项目目录；查看扩展页错误；升级 Chrome |
| Worker 注册失败并提示本地目录或运行时未加载 | 生成产物缺失、过期或构建失败 | 首次先运行 `npm install --ignore-scripts`，再运行 `node scripts/build-provider-runtime.mjs`；确认两个 `lib/` 产物存在后重新加载 |
| 设置页没有自动打开 | 这是重新加载，不是首次安装 | 右键图标选择“打开详细调试面板” |
| 源码已改，但版本仍旧 | Chrome 仍加载旧包或另一份副本 | 对比扩展路径；点击“重新加载”；用运行时四种方法核对版本 |
| 左键后只打开设置页，徽标为 `SET` | 当前 Provider 缺 API Key，或本地目录与已保存模型不一致 | 在设置页重新选择固定模型并填写 Key；若目录异常，重新构建并加载扩展；点击“测试当前服务” |
...
| 修改 `content.js` 后仍执行旧逻辑 | 旧脚本已经注入现有标签页 | 重新加载扩展，再刷新宿主网页，然后重新点击图标 |
...
| 调试面板一直为空 | 只勾选了设置页复选框但没保存，或尚未触发操作 | 右键菜单立即开启，或在设置页勾选后保存；再测试连接/翻译 |
...
- 固定 API 使用精确 `host_permissions`。当前架构不支持运行时增加 Provider、Base URL 或 origin。
- 增加 Provider 时必须同时审查 SDK 依赖、snapshot、schema、allowlist、官方 API host、清单权限和生成产物，不能用通配权限绕过设计边界。
- 所有当前 Provider 都使用 HTTPS；清单没有远程 HTTP 或本机 loopback 权限。

#### 代码与 CSP 边界

- 所有可执行 JavaScript 必须随扩展打包。Vercel AI SDK 依赖在开发阶段构建进 `lib/provider-runtime.js`；扩展运行时不得下载 SDK 或其他待执行逻辑。
