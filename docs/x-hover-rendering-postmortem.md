...
这里的 fresh replacement 指“宿主用文本与语义相同、JavaScript 身份却不同的新 `Element` 替换旧元素”。`generated` 呈现指“可见译文由原文元素的 data 属性和 `::after` 生成，真实文本保存在离屏描述节点中，而不是作为宿主正文的 child/sibling”。本文把这一整组可见属性、离屏描述、ARIA 引用和内存状态简称为 surface。

“排查过程”和“用户环境最终验收”是截至 2026-08-09 的历史记录，不随以后实现改写；“最终设计”“回归保护”和“当前设计边界”描述同日验收版本的维护契约，相关源码变化时必须同步更新。

| 项目 | 结论 |
| --- | --- |
| 用户可见症状 | 鼠标移入 X 帖子或右侧趋势卡片时，译文短暂消失再出现，帖子高度和列表位置随之跳动 |
| 最终根因 | X 的 React 更新会把已翻译正文替换为同文的 fresh `Element`；旧实现把状态绑定在旧元素身份上，只能等待防抖重扫恢复 |
| 最终策略 | X 使用不进入宿主 child tree 的 generated presentation，并在同一 MutationObserver 批次内把状态原子迁移到语义等价的新元素 |
| 自动验证 | 截至 2026-08-09，`npm run check` 全部 204 项通过，生成 runtime 与源码一致 |
...
- class/style 引起的瞬时不可见状态不会清除 generated 译文；source 隐藏时伪元素自然一起隐藏；
...
| 继续向 X 插入真实 child/sibling | React 可能删除未知节点，重新形成删除—恢复反馈环 |
...
| 场景 | 关键断言 |
| --- | --- |
| 帖子与 Show more 连续 80 次 hover | description ID、译文、原始 child identity 和模拟高度保持不变；请求仅 1 次 |
...
| fresh `tweetText` 与整张 fresh article replacement | 下一 MutationObserver turn 已继承原译文和 description ID |
| 宿主先删除全部扩展属性再 replacement | 仍通过 store ownership 迁移 |
| X 右栏 `p` / `h3` 卡片 replacement | 非 `tweetText` surface 同样稳定 |
| 两个同文节点 2→2 replacement | 按收集顺序保留两枚原 description ID |
| 文本或元素自身语言变化 | 不继承旧译文，走正常重翻译或过滤 |
| 停止运行早于 removal record 交付 | detached source、description、data 属性和扩展 ARIA 全部清理 |
...
截至 2026-08-09，最终 `npm run check` 通过 204/204 项，包括 runtime 新鲜度、源码检查、300 行结构约束、契约测试、单元测试和集成测试。按照仓库规则，自动验证没有启动浏览器或视觉自动化。
...
- generated presentation 使用 source 的 `::after`，因此站点策略必须继续排除导航、菜单、tooltip 等非正文或瞬态 UI，避免与宿主 pseudo-element 发生不必要竞争。
...
- [`src/content/dom/generated-presentation.js`](../src/content/dom/generated-presentation.js)：generated 属性、description、ARIA、恢复与清理。
...
- [`chrome-extension/content/content.css`](../chrome-extension/content/content.css)：generated `::after`、离屏 description 与 Hacker News inline 呈现契约。
